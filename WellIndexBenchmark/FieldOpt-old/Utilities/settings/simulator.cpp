...
Simulator::Simulator(QJsonObject json_simulator)
{
    ...
    if (script_name_.length() == 0 && commands.size() == 0)
        throw NoSimulatorCommandsGivenException("At least one "
        "simulator command or a simulator script must be given.");

    // Simulator fluid model
    std::cout << "Contains FluidModel: " <<
                 json_simulator.contains("FluidModel") << std::endl;

    if (json_simulator.contains("FluidModel")) {

        std::cout << "Checking Simulator FluidModel...";

        QString fluid_model = json_simulator["FluidModel"].toString();

        std::cout << fluid_model.toStdString() << std::endl;
