...
WellIndexCalculator::WellIndexCalculator(Model::Reservoir::Grid::Grid *grid, double wellbore_radius)
{
    grid_ = grid;
    wellbore_radius_ = wellbore_radius;
}

QList<WellIndexCalculator::BlockData> WellIndexCalculator::GetBlocks(QList<Model::Reservoir::Grid::XYZCoordinate> points)
{
    ...
    for (int i = 0; i < data.first.size(); ++i) {
        ...
        Model::Reservoir::Grid::IJKCoordinate ijk = grid_->GetCell(global_index).ijk_index();
        block.i = ijk.i();
        block.j = ijk.j();
        block.k = ijk.k();
