...
int main(int argc, char *argv[])
{
          ...
          ii < WellDataPCG_.well_dirs_paths.size(); ii++)
    {
        ...
        // Determine which model the current folder requires
        if( QString::compare(dname.at(0), "n", Qt::CaseSensitive) == 0 ) {
            model = "norne";
            outputf = current_path + "/workflow/NW01.xyz";
            qDebug() << "dname: " << dname
                     << "model: " << model
                     << "outputf: " << outputf;
        }
        else if( QString::compare(dname.at(0), "t", Qt::CaseSensitive) == 0 ) {
            model = "5spot";
        ...
        int ecode = RunRMS(model, current_path,run_rms);
        ...
        if (QString::compare(model,"norne") == 0){
            inputf = current_path + "/workflow/EVENTS_NW01.DATA";
        }
          ...
          ii < WellDataPCG_.well_dirs_paths.size(); ii++)
    {
        ...
        if (inputFile.open(QIODevice::ReadOnly))
        {
            ...
            QVector3D* start_well =new QVector3D(
                first_vec.split(" ").at(0).toFloat(),
                first_vec.split(" ").at(1).toFloat(),
                first_vec.split(" ").at(2).toFloat());
            QVector3D* end_well =new QVector3D(
                second_vec.split(" ").at(0).toFloat(),
                second_vec.split(" ").at(1).toFloat(),
                second_vec.split(" ").at(2).toFloat());

            qDebug() << "QVector start_well = " << *start_well;
            qDebug() << "QVector end_well = " << *end_well;


            // Set up well variables to run well index calculation
            double wellbore_radius = 0.191/2;
            double min_wi = 1e-12;

            // Set up grid variable specific for folder data
            QString file_path_;

            if (QString::compare(model,"norne")  == 0){
                file_path_ = "../../WellIndexBenchmark/model/norne/NORNE_ATW2013.EGRID";
                qDebug() << file_path_;
            }
            else if (QString::compare(model,"5spot") == 0){
                file_path_ = "../../WellIndexBenchmark/model/5spot/ECL_5SPOT.EGRID";
                qDebug() << file_path_;
            }
            ...
            WellIndexCalculator::GeometryFunctions::
            print_well_index_file(grid_, start_points,
                                  end_points, wellbore_radius,
                                  min_wi, filename);
            inputFile.close();
        }
