...
// ROTATE_DISCONTINUOUS TEMPLATE
// =============================
// Rotates two discontinuous ranges to put *first2 where
// *first1 is. If last1 == first2 this would be equivalent
// to

// rotate(first1, first2, last2),

// but instead the rotate "jumps" over the discontinuity

// [last1, first2),

// which need not be a valid range.

// In order to make it faster, the length of
// [first1, last1) is passed in as d1, and d2
// must be the length of [first2, last2).

// In a perfect world the d1 > d2 case would have used
// swap_ranges and reverse_iterator, but reverse_iterator
// is too inefficient.
...
// ROTATE_DISCONTINUOUS3 TEMPLATE
// ==============================
// Rotates the three discontinuous ranges to put *first2
// where *first1 is. Just like rotate_discontinuous, except
// the second range is now represented by two discontinuous
// ranges: [first2, last2) + [first3, last3).
    ...
    typename std::iterator_traits<BidirIter>::difference_type d3)
{
    ...
    if (d1 <= d2)
        rotate_discontinuous(std::next(first2, d2 - d1),
                             last2, d1, first3, last3, d3);
    else
    {
        rotate_discontinuous(std::next(first1, d2),
                             last1, d1 - d2, first3, last3, d3);
        rotate_discontinuous(first2, last2, d2, first3, last3, d3);
    }
}

// COMBINE_DISCONTINUOUS FUNCTION
// ==============================
// Call f() for each combination of the elements
// [first1, last1) + [first2, last2) swapped/rotated
// into the range [first1, last1).

// As long as f() returns false, continue for every
// combination and then return [first1, last1) and
// [first2, last2) to their original state.
// If f() returns true, return immediately.
// Does the absolute mininum amount of swapping to
// accomplish its task. If f() always returns false
// it will be called (d1+d2)!/(d1!*d2!) times.
...
// CALL_COMBINE_DISCONTINUOUS CLASS
// ================================
...
// COMBINE_DISCONTINUOUS3_ FUNCTION
// ================================
    ...
    typename std::iterator_traits<BidirIter>::difference_type d = 0)
{
    ...
    else
    {
        ...
        {
            if (combine_discontinuous(
                f1p, last1, d1-1, i2, last3, d22, f, d+1))
                return true;
            swap(*first1, *i2);
        }
    }
    ...
    if (d != 0)
    {
        if (d2 > 1)
            ...
            rotate_discontinuous(
                first1, last1, d1, first3, last3, d3);
    }
    else
        rotate_discontinuous3(
            first1, last1, d1, first2, last2, d2, first3, last3, d3);
    return false;
}

// COMBINE_DISCONTINUOUS3 CLASS
// // ============================
// Like combine_discontinuous, but swaps/rotates
// each combination out of
...
// If f() always returns false, it is called
// (d1+d2+d3)!/(d1!*(d2+d3)!) times.
template <class BidirIter, class Function>
bool
combine_discontinuous3(
    BidirIter first1, BidirIter last1,
    typename std::iterator_traits<BidirIter>::difference_type d1,
    BidirIter first2, BidirIter last2,
    typename std::iterator_traits<BidirIter>::difference_type d2,
...
// COUNT_EACH_COMBINATION FUNCTION
// ===============================
template <class UInt>
UInt
...
// COUNT_EACH_PERMUTATION FUNCTION
// ===============================
template <class UInt>
UInt
...
// FOR_EACH_CIRCULAR_PERMUTATION FUNCTION
// ======================================
template <class BidirIter, class Function>
Function
